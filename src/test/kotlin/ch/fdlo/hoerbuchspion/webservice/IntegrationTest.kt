...
class IntegrationTest {
    ...
    companion object {
        ...
        fun setup(app: Jooby) {
            ...
            this::class.java.classLoader.getResourceAsStream("queries.sql").bufferedReader().useLines {
                it.forEach {
                    if (it.isBlank() || it.startsWith("--")) return@forEach
    ...
    fun shouldProvideASingleArtistByItsId(serverPort: Int) {
        client.newCall(
            ...
            assertEquals(
                "{\"id\":\"23129390abdc\",\"name\":\"Some Super Fancy Artist\",\"artistImage\":{\"large\":\"http://artist1.com/image_lg.png\",\"medium\":\"http://artist1.com/image_md.png\",\"small\":\"http://artist1.com/image_sm.png\"},\"popularity\":90}",
                rsp.body!!.string()
            )
            assertEquals(StatusCode.OK.value(), rsp.code)
        }
